.<script setup lang="ts">
	...
	**********************************************************/
	...
	const jumpPage = () => {
		...
		return ((JWT == 'undefined') || (JWT == '') || (JWT == null))?'/':'/A03'
	}
	...
	const getSiteInfo = async () => {
		...
		siteIcon.value = window.sessionStorage.getItem('liwaImgsvr') + data.data.value.message
	...
	onMounted(() => {
    	...
    	if (sJWT) {
		    ...
		    usegetJWT().then(res => {
			    ...
			    userIcon.value = window.sessionStorage.getItem('liwaImgsvr') + arrJWT.iconPath
			    bIcon.value = true
			    console.log('userIcon in onMounted =', userIcon.value)        
		    })
	...
	<div class="headPanel w-full h-32 md:h-[4.5rem] bg-gradient-to-r from-indigo-900 to-violet-200 flex flex-col md:flex-row justify-center">
		<div class="logoPanel w-full pt-2 pl-3 md:w-[calc(100%_-_240px)]">
			...
			<div class="float-left">
				<h1 class="text-2xl lg:text-4xl text-white pt-2">{{ siteName }}雲系統</h1>
			</div>
		</div>
		<div class="w-full md:float-right md:w-[240px]">
			<div v-if="bLogin" class="w-full flex flex-row-reverse justify-between mx-auto">
				<div class="sysIcon logout mx-1 mt-1" @click.prevent="logout()">
					<svg width="2rem" height="2rem" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" version="1.1" style="margin-top:5px;">
				...
				<a :href="jumpPage()">
					<div class="sysIcon help mx-1 mt-2">
						<IconQuestionCircleFill class="w-8 h-8 text-red-500 font-bold" />
					</div>
				</a>
				...
				<div v-if="bIcon" class="sysIcon userIcon mx-1 mt-3" @click="goProfile()">
					<img :src="userIcon" class="w-8 h-8" />
				</div>
				<div v-else class="sysIcon userIcon mx-1 mt-3" @click="goProfile()">
					<IconPersonFill class="w-8 h-8 text-slate-400 font-bold" />
				</div>
			</div>
